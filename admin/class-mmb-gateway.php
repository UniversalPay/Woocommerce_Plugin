 ...
 * @subpackage MMB_Gateway_Woocommerce/admin
 */
class Universalpay extends WC_Payment_Gateway {
    /**
    ...
    public function __construct() {
        ...
        $this->method_title = __('UniversalPay', 'mmb-gateway-woocommerce');
        $this->method_description = __('UniversalPay gateway sends customers to UniversalPay to enter their payment information and redirects back to shop when the payment was completed.', 'mmb-gateway-woocommerce');
...
//         $this->api_available_payment_solutions = $this->get_option('api_available_payment_solutions');
        ...
        $api_test_token_url = 'https://api.test.universalpay.es/token';
        $api_test_payments_url = 'https://api.test.universalpay.es/payments';
        $api_test_js_url = 'https://cashierui.test.universalpay.es/js/api.js';
        $api_test_cashier_url = 'https://cashierui.test.universalpay.es/ui/cashier';
        $api_token_url = 'https://api.universalpay.es/token';
        $api_payments_url = 'https://api.universalpay.es/payments';
        $api_js_url = 'https://cashierui.universalpay.es/js/api.js';
        $api_cashier_url = 'https://cashierui.universalpay.es/ui/cashier';
    ...
    /**
     * Check if the currency of store is accepted by UniversalPay
     *
     * TODO: ez kell?
     *
     * @since    1.0.0
    ...
    public function is_valid_for_use() {
        if (!in_array(get_woocommerce_currency(),
            ...
            $this->msg = sprintf(__("UniversalPay doesn't accept your store currency. Check available currencies  %s here", 'mmb-gateway-woocommerce') . "</a>");
            return false;
        }
...
//         $this->form_fields['api_available_payment_solutions']['options'] = $this->get_available_payment_solutions();
    ...
    public function set_wc_notice() {
        if ( isset($_GET['wcapi']) && isset($_GET['order_id'])) {
            ...
            if ('universalpay' == $payment_method) {
...
//             }
    ...
    public function capture_payment( $order_id ) {
        $order = wc_get_order( $order_id );
        if ( $order->get_payment_method() == 'universalpay' ) {
    ...
    public function add_capture_charge_order_action( $actions ) {
        ...
        if ( 'universalpay' !== $payment_method || 'on-hold' !== $payment_status ) {
            return $actions;
        }
        ...
        $actions['universalpay_capture_charge'] = esc_html__( 'Capture Online');
    ...
    public function add_void_charge_order_action( $actions ) {
        ...
        $actions['universalpay_void_charge'] = esc_html__( 'VOID Online');
